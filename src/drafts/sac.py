...
class SAC:
    ...
    def _get_action(self, key: chex.ArrayNumpy,
      ...
      if deterministic:
        return mu
      mu, _ = self._sample_action(key, mu, sigma)
      return mu
    ...
    def _sample_action(self, key: chex.ArrayNumpy,
      ...
      #return rlax.squashed_gaussian().sample(key, mu, sigma, self.environment_spec.actions)
       noise = jax.random.normal(key, sigma.shape)
       action = jnp.tanh(mu + noise * sigma)
       log_std = jnp.log(sigma)
       gauss_log_prob = -0.5 * (jnp.square(noise) + 2 * log_std + jnp.log(2 * math.pi))
       gauss_tanh_log_prob = gauss_log_prob - jnp.log(nn.relu(1.0 - jnp.square(action)) + 1e-6)
       return action, gauss_tanh_log_prob
